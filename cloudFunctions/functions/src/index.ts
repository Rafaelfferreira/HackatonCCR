...
export const getPlacePercentageReviewsUfs = functions.https.onRequest((request, response) => {
    ...
    let lat = request.query.lat
    let lng = request.query.lng
    let ufs = request.query.ufs
    let qtd = request.query.qtd

    let lat_num = Number(lat)
    let lng_num = Number(lng)
    let ufs_str = String(ufs).split('-')
    let qtd_num = Number(qtd)


    var all_data = Array()
    async function grabAllData(): Promise<void> {
        
        for (const uf of ufs_str){
           admin.firestore().collection("placesOpt").doc(uf).collection(uf).get()
                .then(snapshot => {
                        let data = snapshot.docs.map(doc => doc.data())
    ...
    async function wrapperAwait() {
        await grabAllData()
    }

    wrapperAwait()

                if (qtd_num > all_data.length) {
                    qtd_num = all_data.length - 1
                }

                

                let closest = []
                for (const place of all_data.slice(0,qtd_num)) {
                    closest.push({
                    "dist": distance(lat_num, lng_num, 
                        place.coordinates._latitude,
                        place.coordinates._longitude, "K"),
                ...
                for (const place of all_data.slice(qtd_num,)) {
                    let curr_dist =  distance(lat_num, lng_num, place.coordinates._latitude,                            place.coordinates._longitude, "K")
...
export const getFretagem = functions.https.onRequest((request, response) => {

    // reference: https://www.tabelasdefrete.com.br/p/calculo-carreteiro
